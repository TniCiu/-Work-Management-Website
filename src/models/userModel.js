...
const USER_COLLECTION_SCHEMA = Joi.object({
    ...
    password: Joi.string().allow(null).min(6), // Cho phép null
    username: Joi.string().required().min(3).max(30).trim().lowercase(), // Bắt buộc
    displayName: Joi.string().min(3).max(50).trim().optional(), // Tùy chọn
    avatar: Joi.string().uri().trim().allow(null), // Cho phép null
...
const isValidId = (id) => {
    try {
        return ObjectId.isValid(id); // Kiểm tra xem id có phải là ObjectId hợp lệ hay không
    } catch (error) {
        return false;
    }
};
