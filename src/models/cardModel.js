...
const createdNew = async (data) => {
  try {
      ...
      // Tạo `_id` dưới dạng ObjectId nếu chưa tồn tại
      validData._id = new ObjectId();

      // Chuyển đổi `boardId` và `columnId` sang ObjectId
...
const updateCardIdsToObjectId = async (boardId) => {
  try {
      ...
      throw new Error(`Error updating card _id to ObjectId: ${error.message}`);
  }
};
