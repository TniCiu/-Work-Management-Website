...
const CardInformation = ({ board, openCardInformation, onClose, card }) => {
    ...
    useEffect(() => {
        if (card?.attachments) {
            setDescription(card.attachments);
        }
    }, [card]);
    useEffect(() => {
        console.log("Board in CardInformation:", board);
        if (board) {
            fetchInforUserBoardsAPI(board._id)
                .then((res) => {
                    if (res.board && Array.isArray(res.board.members) && Array.isArray(res.board.owners)) {
                        // Kết hợp members và owners
                        const allMembers = [...res.board.members, ...res.board.owners];
    ...
    const handleTitleBlur = async () => {
        if (editedTitle !== card.title) {
            try {
                await updateCardDetailsAPI(card._id, { title: editedTitle }, board._id);
        ...
        if (newComment.trim()) {
            ...
            try {
                ...
                const updatedCard = await updateCardDetailsAPI(
                    card._id,
                    { comments: [...comments, newActivity] },
                    board._id
                );
    
                // Cập nhật danh sách bình luận từ API
...
<Box sx={{ display: "flex", mt: 1 }}>
    {members.map((member) => (
        <Avatar
            key={member._id}
            sx={{ width: 28, height: 28, mr: 1, position: "relative" }}
            src={member.avatar}
        >
            {/* Hiển thị dấu tích nếu thành viên đã được thêm */}
            <CheckIcon
                sx={{
                    position: "absolute",
                    bottom: 0,
    ...
    >
        {boardMembers.map((member) => (
            ...
            >
                ...
                {/* Hiển thị dấu tích nếu thành viên đã có trong card */}
