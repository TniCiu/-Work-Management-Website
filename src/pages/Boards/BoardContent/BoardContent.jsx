...
function BoardContent({ 
  ...
  useEffect(() => {
    // Cập nhật orderedColumnsState khi board thay đổi
    setOrderedColumnsState(board.columns);
  }, [board]);
  useEffect(() => {
  }, [orderedColumnsState]);
  
  // Khi có dữ liệu từ AI, cập nhật mockData cho board hiện tại
  useEffect(() => {
    if (seclectIdeaAI && Array.isArray(seclectIdeaAI.columns) && board._id) {
      setMockData(seclectIdeaAI.columns);  // Cập nhật mockData nếu dữ liệu từ AI là mảng
    }
  }, [seclectIdeaAI, board._id]);
