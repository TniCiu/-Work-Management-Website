...
const createNew = async (req, res, next) => {
    const correctCondition = Joi.object({
        ...
        startDate: Joi.date().iso().required().messages({
            ...
            'date.isoDate': '"Start Date" must be in ISO 8601 format (YYYY-MM-DD).'
        }),
        endDate: Joi.date().iso().required().greater(Joi.ref('startDate')).messages({
            'date.base': '"End Date" must be a valid date.',
            'date.isoDate': '"End Date" must be in ISO 8601 format (YYYY-MM-DD).',
...
const update = async (req, res, next) => {
    ...
    const correctCondition = Joi.object({
        ...
        startDate: Joi.date().iso().messages({
            'date.base': '"Start Date" must be a valid date.',
            'date.isoDate': '"Start Date" must be in ISO 8601 format (YYYY-MM-DD).'
        }).optional(), // Chỉ bắt buộc nếu logic cần
